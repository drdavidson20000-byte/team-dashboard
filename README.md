...
4. 화면은 표(엑셀 형태) 한 장으로 구성됩니다: **분류**(프로젝트) → **항목**(카테고리) →
   **상세설명** 순서로 이어집니다. 같은 분류에 속한 항목들은 분류 셀이 세로로 병합되어
   표시됩니다. 위 **+ 프로젝트 추가**로 분류를 만들고, 분류 셀의 **+ 항목**으로 그 아래
   항목(행)을 추가합니다. 상세설명 칸은 비어 있으면 "클릭해서 작성"이라고 뜨며, 클릭하면
   바로 그 자리에서 입력 후 저장할 수 있습니다.
5. 상단 주차 이동(‹ ›)으로 지난 주 기록도 확인할 수 있습니다. 분류/항목 구조는 주차와
   무관하게 유지되고, 상세설명만 선택한 주차 기준으로 표시됩니다.
6. 내용이 전주와 크게 다르지 않을 때는, 주차 이동 옆의 **"◀ 지난 주 내용 가져오기"**
   버튼을 누르면 이번 주에 아직 비어 있는 항목에만 지난 주 상세설명을 한 번에 채워 넣고,
   이미 작성한 항목은 그대로 둡니다. 채워진 내용은 각 칸을 클릭해서 다시 수정하면 됩니다.
